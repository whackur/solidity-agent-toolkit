...
function checkAderynInstalled(): boolean {
  try {
...
function parseAderynJson(output: string): AderynFinding[] {
  try {
    const parsed = JSON.parse(output);
    // Aderyn output structure: { findings: [...] }
...
function formatFindingsAsMarkdown(findings: AderynFinding[]): string {
    ...
    async ({ path, outputFormat = "json" }) => {
      try {
        ...
        try {
          ...
          // Aderyn may exit with non-zero code even on success, capture output anyway
