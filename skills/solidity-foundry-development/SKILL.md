...
## Enhanced with MCP
Leverage `solidity-agent-toolkit` MCP tools for a streamlined workflow:
- `compile_contract`: Fast compilation and artifact generation.
- `run_tests`: Execute the full test suite.
- `run_single_test`: Target specific test cases for faster iteration.
- `gas_snapshot`: Generate and compare gas reports.
- `dry_run_deploy`: Simulate deployments to catch errors before broadcasting.
