...
- **Common Pitfall**: Reentrancy via `onERC721Received` callback during `safeTransferFrom`. Use `nonReentrant` or follow Checks-Effects-Interactions.
...
- **Inflation Attack**: First depositor can manipulate share price. Prevent by minting "dead shares" to `address(0)` on the first deposit.
...
## Enhanced with MCP
Use the `erc://{standard}` resource from the `solidity-agent-toolkit` to quickly look up full interface definitions and standard-specific documentation.
